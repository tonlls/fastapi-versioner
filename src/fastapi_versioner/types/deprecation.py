    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.is_deprecated and self.deprecation_info:
            ...
            if self.deprecation_info.sunset_date:
                result["deprecation"]["sunset_date"] = self.deprecation_info.sunset_date.isoformat()
                result["deprecation"]["days_until_sunset"] = self.deprecation_info.days_until_sunset
            
            if self.deprecation_info.replacement:
                result["deprecation"]["replacement"] = self.deprecation_info.replacement
            
            if self.deprecation_info.migration_guide:
                result["deprecation"]["migration_guide"] = self.deprecation_info.migration_guide
            
            if self.deprecation_info.reason:
                result["deprecation"]["reason"] = self.deprecation_info.reason
...
class DeprecationPolicy:
    ...
    def __init__(
        ...
        auto_sunset_after_days: Optional[int] = None,
    ...
    def get_auto_sunset_date(self, deprecation_date: datetime) -> Optional[datetime]:
        """
...
# Type aliases
DeprecationLike = Union[bool, DeprecationInfo, Dict[str, Any]]


def normalize_deprecation_info(
    deprecation: DeprecationLike,
