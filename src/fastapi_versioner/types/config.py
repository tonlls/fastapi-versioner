...
from .compatibility import CompatibilityMatrix, CompatibilityMatrixLike, normalize_compatibility_matrix
...
class VersioningConfig:
    ...
    def validate(self) -> None:
        ...
        if self.strategy_priority and set(self.strategy_priority) != set(self.strategies):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "default_version": str(self.default_version) if self.default_version else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> VersioningConfig:
        ...
        if "negotiation_strategy" in config_data:
            config_data["negotiation_strategy"] = NegotiationStrategy(config_data["negotiation_strategy"])
        
        # Parse default version
        if "default_version" in config_data and config_data["default_version"]:
            config_data["default_version"] = Version.parse(config_data["default_version"])
...
class ConfigBuilder:
    ...
    def with_compatibility_matrix(self, matrix: CompatibilityMatrixLike) -> ConfigBuilder:
        """Set the compatibility matrix."""
        self._config_data["compatibility_matrix"] = normalize_compatibility_matrix(matrix)
        return self
...
# Type aliases
ConfigLike = Union[VersioningConfig, Dict[str, Any]]
...
def merge_configs(base: VersioningConfig, override: VersioningConfig) -> VersioningConfig:
    """
