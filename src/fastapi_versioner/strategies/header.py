...
class HeaderVersioning(VersioningStrategy):
    ...
    def extract_version(self, request: Request) -> Optional[Version]:
        """
...
class AcceptHeaderVersioning(VersioningStrategy):
    ...
    def __init__(self, **options):
        ...
        if self.vendor_pattern:
            ...
            self.vendor_regex = re.compile(r"application/vnd\.[\w.-]+\.v(\d+(?:\.\d+)?)\+\w+")
    ...
    def _extract_from_media_type_params(self, accept_header: str) -> Optional[Version]:
        """
    ...
    def extract_version(self, request: Request) -> Optional[Version]:
        ...
        if self.parser_func:
            try:
                ...
                if self.required:
                    raise StrategyError(
                        f"Custom parser failed: {e}",
                        error_code="CUSTOM_PARSER_ERROR"
                    ) from e
...
class MultiHeaderVersioning(VersioningStrategy):
    ...
    def __init__(self, **options):
        ...
        self.strategies = options.get("strategies", [
            HeaderVersioning(),
