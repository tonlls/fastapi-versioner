...
from ..types.deprecation import DeprecationInfo
from ..strategies.base import VersioningStrategy, CompositeVersioningStrategy
from ..strategies import get_strategy
from ..decorators.version import get_version_registry, VersionedRoute, is_versioned
    ...
    def _collect_existing_routes(self) -> None:
        ...
        for route in self.app.routes:
            if isinstance(route, APIRoute) and is_versioned(route.endpoint):
                ...
                versioned_routes = getattr(route.endpoint, "_fastapi_versioner_routes", [])
    ...
    def _setup_version_discovery(self) -> None:
        ...
        async def version_discovery():
            ...
            return {
                ...
                "default_version": str(self.config.default_version) if self.config.default_version else None,
    ...
    def resolve_version(self, request: Request) -> Version:
        ...
        if extracted_version is None:
            ...
            if self.config.default_version is None:
                raise ValueError("No default version configured and no version specified in request")
    ...
    def add_versioned_route(
        ...
        **kwargs: Any
        ...
        for method in methods:
            ...
            versioned_path = self.versioning_strategy.modify_route_path(path, version_obj)
...
class VersioningMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            request.state.version_info = self.versioned_app.versioning_strategy.get_version_info(request)
            ...
            if self.versioned_app.config.raise_on_unsupported_version:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": str(e),
                        "available_versions": [
                            str(v) for v in self.versioned_app.version_manager.get_available_versions()
                        ]
                    }
                )
            else:
                ...
                request.state.api_version = resolved_version
                request.state.version_info = {"version": str(resolved_version), "fallback": True}
        ...
        if self.versioned_app.config.include_version_headers:
            ...
            if hasattr(request.state, "version_info"):
                ...
                if "strategy" in version_info:
                    response.headers["X-API-Version-Strategy"] = version_info["strategy"]
        
        # Handle deprecation warnings
        await self._handle_deprecation_warnings(request, response)
        
        # Add custom headers
        for header_name, header_value in self.versioned_app.config.custom_response_headers.items():
            response.headers[header_name] = header_value
        
        return response
    
    async def _handle_deprecation_warnings(self, request: Request, response: Response) -> None:
        """
        ...
        # Get versioned route
        versioned_route = self.versioned_app.get_route_for_version(path, method, version)
        ...
        if versioned_route and versioned_route.is_deprecated:
            ...
            if deprecation_info:
                ...
                if (self.versioned_app.config.deprecation_policy and 
                    hasattr(self.versioned_app.config.deprecation_policy, 'block_sunset_requests') and
                    self.versioned_app.config.deprecation_policy.block_sunset_requests and 
                    deprecation_info.is_sunset):
                    # This would need to be handled earlier in the middleware chain
                    # For now, we just add a warning header
                    response.headers["X-API-Sunset-Warning"] = "This endpoint has reached its sunset date"
